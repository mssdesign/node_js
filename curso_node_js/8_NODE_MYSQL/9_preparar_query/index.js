...
app.post('/books/insertbook', (req, res) => {
  ...
  const sql = `INSERT INTO books (??, ??) VALUES (?, ?)` //Com preparação de query ?? -> string ? -> variável
  const data = ['title', 'pageqty', title, pageqty] //Substituindo os valores dinâmicos acima na ordem

  pool.query(sql, data, function (err) { //Lembrar de adiconar a constante "data" de segurança do SQL
    if (err) {
      console.log(err)
      return
    }
...
app.get('/books', (req, res) => {
  const sql = "SELECT * FROM books"         //SELECIONANDO TODOS
...
app.get('/books/:id', (req, res) => {
  ...
  const sql = `SELECT * FROM books WHERE id = ${id}`    //FILTRANDO
...
app.get('/books/edit/:id', (req, res) => {  //Pegando dados para edição
  ...
  const sql = `SELECT * FROM books WHERE id = ${id}` //Não precisa de aspas no ID é facultativo  //FILTRANDO SQL
...
app.post('/books/updatebook', (req, res) => {
  ...
  const sql = `UPDATE books SET title = '${title}', pageqty = '${pageqty}' WHERE id = ${id}`  //ATUALIZANDO
...
app.post('/books/remove/:id', (req, res) => {
  ...
  const sql = `DELETE FROM books WHERE id = ${id}`  //DELETANDO
